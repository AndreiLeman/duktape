...
DUK_INTERNAL void duk_err_longjmp(duk_hthread *thr) {
	 ...
	 * is complete and we're ready for the side effects.
	 *
	 * Does not prevent refzero freeing or mark-and-sweep during error
	 * handling.
	 *
	...
	/* FIXME: set this immediately when longjmp state is set */
	DUK_ASSERT(thr->heap->error_not_allowed == 0);  /* Detect error within critical section. */
	thr->heap->error_not_allowed = 1;
#endif

	DUK_D(DUK_DPRINT("about to longjmp, pf_prevent_count=%ld", (long) thr->heap->pf_prevent_count));
